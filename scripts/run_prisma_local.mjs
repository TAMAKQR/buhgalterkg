...
if (!existsSync(localEnvPath)) {
    console.error('Не найден .env.local. Локальная Prisma-команда остановлена, чтобы не подключиться к production по ошибке.');
    process.exit(1);
}
...
if (!localEnv.DATABASE_URL) {
    console.error('В .env.local не задан DATABASE_URL. Prisma-команда остановлена.');
    process.exit(1);
}
...
try {
    ...
    console.error('DATABASE_URL в .env.local имеет неверный формат. Prisma-команда остановлена.');
    process.exit(1);
}
...
if (
    ...
    console.error('Локальные Prisma-команды разрешены только для PostgreSQL на localhost.');
    process.exit(1);
}
